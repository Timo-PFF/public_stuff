...
It's notable that the R-squared values of a linear model ![equation](https://latex.codecogs.com/png.latex?Y=\alphaX+\beta+E) is obviously related to the residual variance ![equation](https://latex.codecogs.com/png.latex?\operatorname{Var}(E)=\operatorname{Var}(Y-\alphaX)). This is easily seen through the definition.
